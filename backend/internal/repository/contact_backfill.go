...
func (repository *ContactBackfillRepository) DB() *gorm.DB { return repository.db }
...
func (repository *ContactBackfillRepository) ListByResolution(resolutionID uint) ([]model.ContactBackfill, error) {
	...
	if err := repository.db.Where("resolution_id = ?", resolutionID).Order("id ASC").Find(&backfills).Error; err != nil {
		return nil, fmt.Errorf("list contact backfills: %w", err)
	}
	return backfills, nil
}

func (repository *ContactBackfillRepository) ListForResolutions(resolutionIDs []uint) ([]model.ContactBackfill, error) {
